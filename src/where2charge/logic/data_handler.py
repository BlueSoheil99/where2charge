"""This module contains functions to download, process, and save data 
for EV charging stations and census tracts."""
...
def _get_data(adr_prefix):
    """Download EV charging station and registration data."""
    url1 = (
    "https://developer.nrel.gov/api/alt-fuel-stations/v1.csv?"
    "access=public&api_key=foEpOo7RpC4gPM41vxhvNB8IQLzek39WVbwjlX5p"
    "&cards_accepted=all&cng_fill_type=all&cng_has_rng=all"
    "&cng_psi=all&country=US&download=true&e85_has_blender_pump=false"
    "&ev_charging_level=2%2Cdc_fast&ev_connector_type=all&ev_network=all"
    "&fuel_type=ELEC&funding_sources=all&hy_is_retail=true&limit=all"
    "&lng_has_rng=all&lpg_include_secondary=false&maximum_vehicle_class=all"
    "&offset=0&owner_type=all&state=all&status=E%2CT&utf8_bom=true"
    )
    ...
    if response1.status_code == 200:
        ...
        raise Exception(
            f"EV charger data failed to download file."
            f"HTTP Status Code: {response1.statuscode}"
            )
    if response2.status_code == 200:
        with open(output_file2, 'wb') as file:
            file.write(response2.content)
        print(
            f"EV registration data downloaded successfully as '{output_file2}'."
            )
    else:
        raise Exception(
            f"EV registration data failed to download file."
            f"HTTP Status Code: {response2.statuscode}"
            )
    return pd.read_csv(output_file1), pd.read_csv(output_file2)


def _get_geo_data(evcs_df, adr_prefix):
    """Get the geodata for the EV charging stations."""
    try:
...
def _merge_data(geo_df, df_EV, adr_prefix):
    """Merge the EV charging station data with the geo data."""
    try:
        ...
        merged_df = merged_df[['Fuel Type Code', 'Station Name', 'Street Address',
                               'Intersection Directions','City', 'Station Phone',
                               'Groups With Access Code', 'Access Days Time',
                               'EV Level1 EVSE Num', 'EV Level2 EVSE Num',
                               'EV DC Fast Count', 'EV Other Info',
                               'EV Network', 'Latitude', 'Longitude',
                               'ID', 'Open Date', 'EV Connector Types',
                               'Facility Type', 'EV Pricing', 'Restricted Access',
                               'EV Workplace Charging', 'EVs']]
...
def _push_data(adr_prefix):
    """Push the processed data to GitHub."""
    today = datetime.today().strftime("%Y-%m-%d %H:%M:%S")
    os.system(f"git add {adr_prefix}/data/merged_df.csv")
    os.system(f"git commit -m 'Uploading the final version of the data - {today}'")
    os.system("git push origin main")


def get_supplementary_data(address_prefix='src/where2charge/logic', generate=False):
    """Get the data for the EV charger recommendation system."""
    if generate:
